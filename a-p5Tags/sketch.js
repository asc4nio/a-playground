...
function setup() {
    ...
    hand.centerV = createVector(width / 2, height / 2)
}
...
var hand =  {
    update: function(){
        this.pointerV = createVector(mouseX, mouseY);
        this.relativeV = this.pointerV.sub(this.centerV)
        this.rotFromRelV = this.relativeV.heading() - PI * 0.5
    },
    show: function() {
        push()
        translate(width / 2, height / 2)
        rotate(this.rotFromRelV)
        translate(0, this.relativeV.mag())
        translate(-90, -10)
        image(this.image, 0, 0, this.image.width * 0.5, this.image.height * 0.5)
        pop()
    }
